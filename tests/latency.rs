...
use ride_engine::{BufferSession, CompletionContext, CompletionQuery, Engine, Lang, QueryMode};
...
fn completion_p95_under_5ms_per_site() {
    ...
    for (name, rel, after, typed) in sites {
        let got = complete_p95(&engine, &manifest().join(rel), after, typed);
...
fn editor_queries_p95_under_1ms() {
    let text = std::fs::read_to_string(manifest().join("samples/rust-demo/src/main.rs")).unwrap();
