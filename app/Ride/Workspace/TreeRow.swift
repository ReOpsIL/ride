...
struct TreeRow: View {
    ...
    var body: some View {
        let icon = mark == .none
            ? FileIcon.spec(name: node.name, isDirectory: node.isDirectory, expanded: expanded, chrome: ts.chrome)
            : FileIcon.project(chrome: ts.chrome)
    ...
    private func menu(for node: FileNode) -> some View {
        ...
        if mark != .none {
            Button("Set as Active Project") { state.projectModel.choose(root: node.url.path) }
                .disabled(mark == .active)
            Button("Build Project") {
                state.projectModel.choose(root: node.url.path)
