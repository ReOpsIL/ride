...
fn name_of(param: &str, index: usize) -> String {
    if let Some((pat, _)) = param.split_once(':') {
        ...
        if pat.chars().all(is_word) && !pat.is_empty() {
...
mod tests {
    ...
    #[test]
    fn empty_name_anchors_on_the_first_paren() {
    ...
    #[test]
    fn name_match_skips_earlier_mentions() {
        let params = parse("fn map_ok(map: Map, f: F)", "map").unwrap();
        assert_eq!(params.open, 9);
    }
}
