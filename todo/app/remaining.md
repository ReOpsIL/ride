...
# Self-test undo flake (added 2026-09-20, seen across batch 3 merges)

- The `* undo` steps (`inline undo`, `intention undo`, `breakpoint shift undo`, once `constant undo`) fail intermittently in full runs and pass alone; a run at `249d519` was 128/0, later runs on the same tree 3 FAIL, the C++ `inline undo` too. Evidence from an instrumented run: the breakpoint undo reverted the text with NO `textDidChange` delivered to the coordinator, so nothing shifted; with NSLog instrumentation in `textDidChange` the same sequence passed 4/4. Polling the check (`until:` with a 10 s timeout) does not help: the undo genuinely does not go through the edit path. Suspects: `NSTextView` undo replaying against a text storage that another path rewrote (`BufferDocument.bind` sets `textView.string` directly; `PendingDelivery`), a live-check or intention-gutter callback landing between edit and undo, or an open undo group at `undo()` time (`groupingLevel` was 1 in one instrumented run). Reproduce with `--only` of the intention group followed by the breakpoint group; log `undoManager.groupingLevel`, `canUndo` and `undoActionName` before `undo()` and every `bind` with a stack.
