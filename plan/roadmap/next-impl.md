...
- `cargo test` configures CMake into `samples/cpp-demo/build/Debug/` (gitignored); that generated database has absolute paths into the checkout and shadows the committed `build/compile_commands.json`, so copy the sample for a self-test run from a clean tree or delete `samples/cpp-demo/build/Debug` first.
- Live debugging needs macOS developer mode (`DevToolsSecurity -status`); when it is disabled, any `lldb` process launch blocks on a system dialog forever. Tests that launch a debuggee check the status first and skip with a message. The maintainer enables it once with `sudo DevToolsSecurity -enable`.
...
### R24 Catch2 coverage, locations and durations (R21) — Tier A

Engine only, `src/run/tests/catch2/` and its fixtures. (1) Add a test case with two failed `CHECK`s and no section to the Catch2 program, recapture `catch2-run.xml` with the same v3.16.0 build, and assert one event whose output lists both expressions. (2) Switch the list command to `--reporter xml --list-tests` (the real binary emits `<SourceInfo><File>` and `<Line>` there) and parse it with the same scanner so `TestCase.file` and `line` are filled; recapture `catch2-list.xml` and drop the text fixture. (3) Add `--durations yes` to the run argv and fill `duration_ms` from `durationInSeconds`. (4) A truncated-stream test: feed the fixture cut in the middle of a test case and assert the earlier events and no panic. Keep files under ~200 lines.

### R25 Stale run lines dropped everywhere (R23, T2) — Tier B

Three leftovers on commit 9861b06. (1) `BuildSession.line(runId:_:)` returns the raw line when the id is stale while a session is active; it must return `nil` in that case (drop) and pass the line through only when no session is active; `BuildSessionStateTests` distinguishes the two. (2) `buildStopAndRerun` in `SelfTestSteps+Run.swift` injects the stale line after both builds finished, where `RunOutput.append` already rejects it; make the step exercise the real race: start build A, begin the stop-and-rerun so `BuildSession` has advanced to B while `RunOutput.runId` is still A, inject a line with A's id and assert it never reaches B's diagnostics or the panel; alternatively drive it through `ProcessRunner` with a script that prints after receiving SIGTERM. (3) `TestSession.line(_:)` has no run id and gates only on `isActive`, so a stopped test run's trailing output merges into the next run's results; give `TestSession` the same `begin(runId:)`/`line(runId:_:)`/`finish(runId:status:)` shape backed by `BuildSessionState`, wire it through the line filter in `AppState.swift`, and add a `TestTreeTests` or `BuildSessionStateTests` case plus a self-test step for a test stop-and-rerun.

### R26 Tests panel framework and names (T2) — Tier B, after R25

Four findings on commit 7f1b9a3. (1) `AppState+Tests.swift` derives the session's framework from the project kind only, so a gutter click on a GoogleTest or Catch2 marker in a Make or compile-database project runs the binary but never opens or streams the Tests panel; the framework comes from the marker (or the target's known framework) and falls back to the project kind only for Run Tests. (2) `TestTree.failedNames` returns leaf names, so Rerun Failed for GoogleTest builds `--gtest_filter=<leaf>` instead of `Suite.Leaf`; qualify with the suite per framework. (3) In `src/run/tests/cargo.rs` the event `name` keeps the full `util::counts_one` path while `suite` is also `util`, so rows duplicate the suite; strip the suite from `name` like `gtest.rs` does and adjust the cargo filter argv so Rerun Failed still passes the qualified path; update `tests/test_output.rs` and the self-test assertion. (4) `src/run/tests/markers.rs` ignores block comments, so `#[test]` inside `/* */` yields a marker; track block comment state in the scan with a test. Acceptance: `TestTreeTests` for (2), engine tests for (3) and (4), self-test on `samples/cpp-demo` unchanged, self-test on `samples/rust-demo` shows the row name `counts_one` under suite `util`.

### R27 Debug session lifecycle (D3) — Tier B

Four findings on commit 339ef6f, engine only. (1) `disconnect` in `src/debug/session/requests.rs` only forwards a blocking request; when the adapter never answers, the state must still become `Terminated`, the event pump must stop, and the `Transport` must be dropped so the child is killed; `debug_command(Disconnect)` removes the session only after that, and a test with a fake adapter script that ignores `disconnect` proves the child dies within the timeout. (2) Sessions that terminate on their own (debuggee exit, adapter crash, handshake failure) stay in `Inner.debug_sessions`; remove on any transition to `Terminated`/`Exited` and never reuse an id (monotonic counter). (3) `handshake.rs` decides between `Running` and `Stopped` by reading the state before the event pump has started, so a `stopped` event queued with the launch response yields a spurious `Running`; drain the queued events before emitting, or start the pump before awaiting the launch response; the scripted test asserts event order for a stop-on-entry launch. (4) The FFI record `Scope` collides with the crate-root `extract::Scope` re-export; rename it `DebugScope` (Swift consumers use `DebugScope`).

### R28 Debug session races (R27) — Tier B, after D5

Two races on commit e8f3e0f. (1) `shutdown` in `src/debug/session/mod.rs` and `terminate` in `events.rs` both check `finished()` then write `Terminated`, so a normal disconnect (adapter answers with a `terminated` event while the caller tears down) can emit `Terminated` twice; make the transition a single compare-and-set on the state (a `Mutex`-guarded `transition(from, to) -> bool` or an atomic state) and emit only when it succeeds; a scripted test asserts exactly one `Terminated` and exactly one registry removal. (2) The 250 ms settle loop in `handshake.rs` still lets a `stopped` event that arrives later than the settle window produce a spurious `Running`; close it deterministically: start the event pump before awaiting the launch response, have the transport stamp every received message with a monotonic receive counter, and after the launch response wait until the pump has processed every message received up to that point (compare counters) before deciding `Running`; remove the settle loop. Extend the fake adapter script with a delayed `stopped` (after the launch response by 400 ms) and assert no `Running` is emitted before it. Also cover an `Exited` transition removing the session from the registry.

### R29 Debug panel loads off the main thread with a stop generation (D5) — Tier B

`DebugPanelModel+Load.swift` calls `debugThreads`, `debugStack`, `debugScopes`, `debugVariables` and `debugEvaluate` synchronously on the main thread, as do `refreshWatches` and the hover path in `DebugHover.swift` / `HoverController.swift`. Fix: `DebugPanelModel` keeps a `stopGeneration` incremented on every `Stopped` event and cleared on continue/terminate; every load runs on a utility queue (the pattern in `DebugFilters.swift`), carries the generation it started with, and publishes on main only when the generation still matches; loads for the same key coalesce so a burst of stops does not queue stale work; watches evaluate concurrently and publish as a batch; the hover evaluation runs off-main with the same guard and shows nothing if the process resumed meanwhile. Extract the generation and coalescing logic into a pure `DebugLoadState.swift` (RideTests: a result with an old generation is dropped, a matching one is applied, a duplicate in-flight key is coalesced). Extend the fake-adapter-backed engine tests only if needed; the self-test steps stay as they are.

### R30 Failed emit gated and drain timeout terminates (R28) — Tier A, after D6

Engine only, `src/debug/session/{mod,handshake}.rs`. (1) `fail` emits `DebugEvent::Failed` before its compare-and-set to `Terminated`, so a session torn down while the handshake is in flight can call the listener after termination; emit `Failed` only when the transition succeeds. (2) When the launch-decision thread's drain wait times out (the pump died), the session stays `Launching`; treat the timeout as a failure: transition to `Terminated`, emit `Failed`, shut the transport down and retire. Tests with the fake adapter: a disconnect during the handshake yields exactly one terminal event; a script that kills the adapter's stdout mid-handshake yields `Failed` once within the drain timeout and removes the session from the registry.

### R31 Debug launch honours the engine sysroot (D6) — Tier A, after R30

`src/debug/render.rs` calls `rustc --print sysroot` on every Rust launch and ignores `EngineConfig.sysroot`, which every other consumer treats as the override and fixture point. Thread the resolved sysroot (the value `Engine::new` already computes through `discover::sysroot_path`) from `Engine::debug_launch` through `DebugRegistry::start` and `DebugSession::start` into the handshake, so `render::init_commands` receives it and no process is spawned per launch. Also make `is_rust_target` require a `Cargo.toml` in the cwd or an ancestor, or a `target/<profile>/` path whose parent holds a `Cargo.toml`, so a C++ project with a directory named `target` is not misdetected; tests for both in `tests/debug_render.rs` (a fixture sysroot passed through `EngineConfig.sysroot` is what the launch request carries).

### R32 Live debugging against the real adapter (D3–D6) — Tier B

Developer mode is now enabled on the maintainer's machine, and the first live run shows two failures. (1) `tests/debug_render_live.rs::the_live_session_summarises_rust_values` times out after 30 s waiting for the `Stopped` event: a temp crate with a breakpoint on `src/main.rs:4`, launched through `DebugSession::start` with the real `lldb-dap`; meanwhile `tests/debug_session.rs::the_live_adapter_stops_in_the_rust_demo` passes in about a second, so compare the two launches (working directory, `stopOnEntry`, breakpoint path form, whether the breakpoint is verified in the `setBreakpoints` response, whether the launch response arrives, whether `stopped` arrives with a different reason such as `entry` and is consumed before the wait starts) and fix the session, not the test. (2) The app self-test steps `debug` and `debug step over` fail with `line 0 stopped false`: Debug builds and launches `samples/rust-demo` with a breakpoint on line 10 but the controller never reports a stop; instrument with `FileHandle.standardError` prints during the investigation (remove them after), capture the DAP frames the app exchanges (add a `RIDE_DAP_TRACE=<file>` environment switch in the transport that appends every frame, kept as a permanent debugging aid), and fix the root cause in the engine or the app. Acceptance, all run with developer mode on: `cargo test --test debug_session --test debug_render_live` passes three times in a row; the Rust self-test scene reports 0 FAIL with the `debug` and `debug step over` steps live (stop on line 10, step to 11); `lldb-dap` launched by the app is gone after Stop (`pgrep lldb-dap` empty).

### S2 Demo scenes and a screenshot pipeline for every feature and language — Tier B

Screenshots in `README.md` were captured by hand from the demo scenes in `app/Ride/Debug/DemoScene.swift`, and the 1.2 panels have no scenes. (1) Add scenes, each staging state in-process without synthetic input, one per feature and language: `targets` (sidebar Targets section on `samples/rust-demo`), `run` (Run Output after `cargo run`), `tests` (Tests panel after Run Tests), `terminal` (one terminal tab open), `debug` (stopped on `src/main.rs:10` with the Debug panel, locals expanded one level, one watch; needs developer mode, which is on), `split` (two panes, `src/main.rs` and `src/util.rs`), `quickdoc` (⌃J popup on `Counter`), `peek` (⌥Space on `record`), `c` (`samples/c-demo` source with member completion), `cmake` (`samples/cpp-demo/CMakeLists.txt` with its outline), `toml` (`Cargo.toml` with the manifest completion), and keep the existing `editor`, `completion`, `cheatsheet`, `hover`, `problems`, `symbols`, `find`, `preview`, `light`, plus a `cpp` and `makefile` scene if they are today produced through `file --file`. Every scene opens with `--frame 1440x900`. (2) `scripts/screenshots.sh`: builds the Debug app, then for each scene copies the sample it needs to a temp dir, launches `Ride --demo <scene> --open <copy> --frame 1440x900`, waits for the scene (a `--ready-file <path>` flag the scene touches when staged; add it to `DemoLaunch`), captures the window with `screencapture -l <CGWindowID> -o <png>` where the window id comes from a small `scripts/windowid.swift` using `CGWindowListCopyWindowInfo` filtered by owner `Ride`, quits the app through `NSApp.terminate` (a `--quit-after <seconds>` flag) and never `kill -9`, and writes `docs/images/ride-<scene>.png`. It must verify each capture is not blank (a PNG whose pixels are all one colour means the Screen Recording permission is missing: stop and report exactly that). (3) Update `README.md`: one image per feature and language in the order the text introduces them, captions in the existing style, the debug screenshot replaced by the scene capture; keep the prose accurate to what the scene shows. Acceptance: `scripts/screenshots.sh` runs end to end on this machine, every `docs/images/ride-*.png` is regenerated and non-blank, the README references only images that exist, and the app tests still pass.

### S3 Screenshot polish and Quick Definition ranking (S2) — Tier B

Three findings from the S2 captures. (1) The `debug` scene hides the Run Output panel once the build finished, gives the Debug panel at least 320 pt (persist through the same height mechanism as the other panels), expands the `counter` local one level and selects the top frame, so the capture shows frames, locals and the watch like the maintainer's manual screenshot. (2) The sidebar section header for Targets wraps its title vertically at the default sidebar width (`app/Ride/Project/TargetsPanel.swift`); the header lays out like the Outline header (title, count badge, single line, truncating). (3) `quick_definition` and `find_definitions` (`src/engine/symbols.rs`) return every catalog hit for a common name, so Quick Definition on `record` shows a dozen truncated `impl` segments and `Counter` resolves to a crate's `StreamCipherCounter`; rank workspace and buffer hits first, then reachable headers, then catalog items whose crate is imported by the workspace, and cap the peek at eight segments with a "more" affordance; add a `tests/definition.rs` case on `samples/rust-demo` asserting `Counter` resolves to `util::Counter` first. Then re-run `scripts/screenshots.sh` for `debug`, `targets`, `peek` and `quickdoc` and commit the images. Acceptance: the three images regenerated and non-blank, app tests green, the Rust self-test still `EXIT 0`.
...
## 12. Release 1.2 cards — debugger on lldb-dap (2026-09-12)

Design, fixed by the strong model: the engine owns the Debug Adapter Protocol client (`src/debug/`) and exposes a session to the app through commands plus a `DebugListener` callback trait, the same shape as `IndexStatusListener` in `src/ffi/index.rs`. `lldb-dap` comes from `xcrun -f lldb-dap` (Xcode) or PATH; no other adapter in 1.2. The app never parses DAP itself.

### D1 DAP transport and framing — Tier A

`src/debug/transport.rs`: spawn the adapter with piped stdio, a writer that frames JSON with `Content-Length: N\r\n\r\n`, a reader thread that parses frames (tolerating extra headers and CRLF/LF) and pushes `serde_json::Value` messages onto a channel, a `seq` counter, `request(command, arguments) -> Result<Value, EngineError>` that correlates on `request_seq` with a timeout, and `events()` as an iterator over event messages. `src/debug/adapter.rs` finds `lldb-dap` (`crate::toolchain::find_tool` plus the `xcrun -f` fallback used for clang). Tests in `tests/debug_transport.rs` against a tiny fake adapter written as a Rust test helper binary (`tests/bin/fake_dap.rs` via `[[bin]]` in `Cargo.toml`, `required-features` none) that echoes an `initialize` response and emits one `output` event; a truncated frame must yield an error, never a panic.

### D2 DAP protocol types — Tier A

`src/debug/protocol/` with serde structs for the requests, responses and events Ride uses: `initialize`, `launch` (program, args, cwd, env, `stopOnEntry`, `initCommands`), `attach` (pid), `setBreakpoints` (source path plus `SourceBreakpoint { line, condition, hitCondition }`), `setExceptionBreakpoints` (`cpp_throw`, `cpp_catch`, and the Rust panic filter lldb-dap exposes), `configurationDone`, `threads`, `stackTrace`, `scopes`, `variables` (with `variablesReference` paging), `evaluate` (context `watch`/`hover`/`repl`), `continue`, `next`, `stepIn`, `stepOut`, `pause`, `disconnect`; events `initialized`, `stopped` (reason, threadId, hitBreakpointIds), `continued`, `exited`, `terminated`, `output` (category, output), `thread`, `breakpoint`. Each type round-trips through JSON in `tests/debug_protocol.rs` using fixtures captured from a real `lldb-dap` session on a debug build of `samples/rust-demo` (start the adapter, send initialize/launch with `stopOnEntry`, threads, stackTrace, scopes, variables, disconnect; store the raw frames under `tests/fixtures/dap/`). Files under ~200 lines: one file per group.

### D3 Debug session state machine — Tier B, after D1 and D2

`src/debug/session.rs`: `DebugSession` drives the handshake (`initialize` → wait `initialized` → `setBreakpoints` for every file → `setExceptionBreakpoints` → `configurationDone` → run), keeps `state: Idle | Launching | Running | Stopped { thread, reason } | Exited(code) | Terminated`, caches threads and the selected frame, and maps every incoming event to a `DebugEvent` for the listener. Breakpoints are stored per file in the session (`add`, `remove`, `toggle`, condition and hit count edits re-send `setBreakpoints` for that file, also while running). `src/ffi/debug.rs`: `DebugLaunch { program, args, cwd, env, stop_on_entry }`, `Breakpoint { path, line, condition, hit_condition, verified }`, `StackFrame { id, name, path, line, column }`, `Scope`, `Variable { name, value, type_name, variables_reference, children_count }`, `DebugEvent` enum, and `trait DebugListener { fn on_event(&self, event: DebugEvent); fn on_output(&self, category: String, text: String); }`. The Xcode 26 `lldb-dap` exposes no `rust_panic` exception filter (only `cpp_throw`, `cpp_catch` and the ObjC/Swift ones), so the session checks `Capabilities::has_filter` before sending a filter and sets a function breakpoint on `rust_panic` for Rust targets instead. `Engine::debug_launch(launch, listener) -> session id`, `debug_command(session, command)` for continue/next/step in/out/pause/disconnect, `debug_threads`, `debug_stack(thread)`, `debug_scopes(frame)`, `debug_variables(reference)`, `debug_evaluate(frame, expression, context)`, `debug_set_breakpoints(path, [Breakpoint])`. Transport fixes from the D1 review come with this card: `Transport::request` wraps a write failure with the command name like `await_response` does; the `tests/debug_transport.rs` closed-adapter case must pass deterministically; `src/lib.rs` re-exports only what tests and the engine use; the event channel is drained continuously by the session's reader loop (never batch polls) and responses whose request already timed out are evicted from the inbox. Tests in `tests/debug_session.rs` on a debug build of `samples/rust-demo`: launch with a breakpoint on `src/main.rs:10`, expect `Stopped` at line 10, `debug_stack` names `main`, `debug_variables` on the locals scope lists `counter`, `next` moves to line 11, `disconnect` terminates; skipped when `lldb-dap` is absent.

### D4 Breakpoints in the gutter and the debug toolbar — Tier B, after D3 and T2

App: `app/Ride/Debug/DebugController.swift` (owns the session id, implements `DebugListener` and republishes on the main queue), `Breakpoints.swift` (pure, RideTests: per-file set with toggle, condition, hit count, persistence in `WorkspaceState.breakpoints`), gutter click on the line-number area toggles a breakpoint (a fold chevron click or a test marker click keep their meaning; the breakpoint glyph is a red dot, hollow when unverified), right-click edits condition and hit count; the current stopped line gets a highlight rendering attribute through `HighlightApply`. `Debug` menu: Debug (⌃⌘R, uses the Q3 target and run config, builds first through the Build action then launches with the target's `run` argv program), Continue (⌘⌥R), Step Over (F8), Step Into (F7), Step Out (⇧F8), Pause, Stop (⌘F2), Toggle Breakpoint (⌘F8), Exception breakpoints toggles; all through `MenuModel`. Self-test on `samples/rust-demo`: set a breakpoint on line 10, Debug, wait for `Stopped`, assert the stop line, Step Over, assert line 11, Stop.

### D5 Debug panel — Tier B, after D4

Engine first: `DebugLaunch` gains `exception_filters: Vec<String>` (the app passes the enabled ones among those the adapter advertises, read from a new `Engine::debug_exception_filters() -> Vec<ExceptionFilter { id, label, default }>` that runs a throwaway `initialize` against the adapter and caches the answer), and the Debug menu's exception toggles left out of D4 use it. Then the panel: `app/Ride/Debug/DebugPanel.swift` as a bottom panel tab: a threads and frames list (click selects the frame and jumps the editor), a variables tree (`VariableTree.swift` pure, RideTests: lazy children by `variablesReference`, paging over 100 children), a watches list (`evaluate` with context `watch` on every stop), and an Evaluate Expression sheet (⌥F8) with `repl` context; hover in the editor while stopped evaluates the identifier under the mouse with context `hover` and shows the value in the existing hover card. Persist watches per workspace.

### D6 Rust and C++ value renderers — Tier B, after D5

`src/debug/render.rs`: `lldb-dap` `initCommands` that import the Rust formatters shipped with the toolchain (`command script import` of `rustc`'s `lldb_lookup.py` under `$(rustc --print sysroot)/lib/rustlib/etc/`, found through `src/discover/sysroot.rs`) so `String`, `Vec`, `Option`, `HashMap` and `&str` show summaries; libc++ formatters are built into lldb. Engine tests in `tests/debug_render.rs` assert the summary strings for a `Vec<u32>` and an `Option<String>` local in a temp crate; app: the variables tree shows the summary as the value and expands into children.
...
| 11 | D1, D2 in parallel; then D3; then D4–D6 | D1 and D2 merged (D2 fixtures captured from lldb-dap 21.0.0 via a core file because developer mode is off); D3 merged; R27 merged and reviewed: R28 after D5; D4 merged (301 app tests, self-test 86/86 with the live debug step skipped); D5 merged (307 app tests, self-test 89/89) and reviewed: R28 and R29 merged and reviewed; D6, R30 and R31 merged; R31b merged and reviewed. Release 1.2 complete on 2026-09-12 at 4c55129: fmt, clippy, 346 engine tests, 311 app tests, self-test 89/89 Rust and 32/32 C++. Developer mode was enabled on 2026-09-12; R32 fixed breakpoint path canonicalisation and live debugging is verified: engine live tests pass, the Rust self-test scene reports 91/91 with a real breakpoint stop on line 10, Step Over to 11 and Stop, and no `lldb-dap` is left behind. |
