...
| P-8b | The seven oversized files split by responsibility (generate per kind, self-test arrays per group, `RideTextView+Apply`, `AppState+Init`/`+Tree`, `DetailColumn`), `scripts/check-lines.allow` empty; pure move verified by review | merge |
| P-6 | `lineEndings` preference (keep / lf) with a status-bar menu, tree New File with a language popup defaulting by project kind, Move Statement Up/Down (⇧⌘↑/↓) through `Engine::statement_bounds`; Grok executor, Sonnet review; follow-ups in `todo/app/remaining.md` | merge |
| P-2 | Welcome "Set up" section (Command Line Tools, rustup, rust-src, cmake, developer mode with Copy, never sudo), samples bundled as resources with "Try a sample project", Tools sheet opens on one tool, manual rows can only be copied | merge |
| P-5 | C++ debug self-test (stop in `Circle::area`, frames, `std::vector` local, Step Out), C self-test array on `samples/c-demo`, CI enables developer mode and runs three scenes; root-cause fix: `DebugProgram` resolved relative program paths, so Debug never stopped for CMake, Make and compile-db targets | merge |
| P-4 | `ride [path[:line[:column]]]` helper in `Contents/Helpers`, `ride://open` URL scheme and document types handled by `application(_:open:)`, workspace root finder, Preferences › Tools installer for `/usr/local/bin/ride` | `a38976b` |
...
Final verification on `main` `a7af69c` (2026-09-20): 467 engine tests, 443 RideTests, `check-lines.sh` clean, self-tests alone: Rust 140/0, C++ 70/0, C 23/0. Release 1.0 public is code-complete; tagging `v1.0.0` triggers `release.yml` (signing, notarization and the Sparkle appcast need the repository secrets it documents; without them it publishes an ad-hoc-signed zip).
