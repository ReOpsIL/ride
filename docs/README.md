...
|---|---|---|---|---|---|---|
| Rust | `rs` | tree-sitter-rust | `extract` items | keywords, buffer locals, crate catalog, `use` and `::` paths, typed members after `.` (buffer types + catalog), struct-literal fields | buffer outline + catalog | rustfmt |
