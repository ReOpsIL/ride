...
| `src/cheatsheet` | language cheat sheets: TOML sections under `cheatsheets/<lang>/` embedded per language (`sheets/`), parsed once (`load.rs`), selected by caret context and typed prefix (`lookup.rs`), placeholders validated like the app parses them (`stops.rs`) |
