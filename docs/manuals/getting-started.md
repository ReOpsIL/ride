...
5. **Debug.** Click a line number to set a breakpoint, then `⌃⌘R`. F7 / F8 / `⇧F8` step; `⌘F2` stops. The Debug panel (`⌘3`) shows frames, locals, watches, and Evaluate Expression (`⌥F8`). Hover an identifier while stopped to see its value.
